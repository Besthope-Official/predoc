...
class TestPDFProcessing:
    ...
    def processor(self, test_env, temp_test_dir, mock_oss_config) -> PDFProcessor:
        upload_to_oss = os.getenv(
            'TEST_UPLOAD_TO_OSS', 'false').lower() == 'true'
    ...
    def test_parse_stage_benchmark(self, pdf_files: List[Path], processor: PDFProcessor, benchmark):
    ...
    def test_chunk_stage_benchmark(self, parsed_text: str, processor: PDFProcessor, benchmark):
    ...
    def test_embedding_stage_benchmark(self, chunks, processor: PDFProcessor, benchmark):
    ...
    @pytest.mark.skip
    def test_single_pdf_parsing_benchmark(self, pdf_files: List[Path], processor: PDFProcessor, benchmark):
        def pdf_processing():
            pdf_file = pdf_files[0]
    ...
    def test_preprocess_with_oss_benchmark(self, pdf_files: List[Path], benchmark):
        ...
        try:
            ...
            if 'test-prep-perf' not in [bucket.name for bucket in client.list_buckets()]:
                pytest.fail(
                    "MinIO test bucket 'test-prep-perf' does not exist.")
